...
start_date = init_time_now - timedelta(days = 90)
start_date = start_date.strftime('%Y-%m-%d-%H-%M')
end_date = (today)
new = HistoricalData('BTC-USD',3600,start_date,end_date).retrieve_data()
new = new.reset_index()
...
model = AutoTS(forecast_length=6, frequency='infer', ensemble='simple', transformer_list="fast", model_list="fast_parallel")
...
for index, row in df.iterrows():
	cursor.execute("INSERT INTO BITCOIN_v1_COINBASE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'BTC','DAG-on_CB-F-90D','Hourly')", row.time, row.close)
cnxn.commit()
cursor.close()