...
start_date = init_time_now - timedelta(days = 14)
...
model = AutoTS(forecast_length=8, frequency='infer', ensemble='simple', max_generations=5)
...
for index, row in df.iterrows():
	cursor.execute("INSERT INTO BITCOIN_v1_COINBASE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'BTC','BAG-on_CB-S-14D','Hourly')", row.time, row.close)
cnxn.commit()
cursor.close()