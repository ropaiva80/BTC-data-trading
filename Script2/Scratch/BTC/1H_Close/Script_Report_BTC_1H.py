...
df = pd.read_sql_query('SELECT * FROM BITCOIN_v1_COINBASE', cnxn)
df1 = df
...
datetime1 = init_time_now - timedelta(hours = 2)
datetime1 = datetime1.strftime('%Y-%m-%d %H:%M:%S')
datetime2 = init_time_now + timedelta(hours = 5)
...
grouped_prediction_final1 = (grouped_prediction.groupby(['ALGORITHMS', pd.Grouper(key='DATETIME_PREDICTION')]).agg({'PRICE_PREDICTION':'mean'}))
grouped_prediction_final1 = grouped_prediction_final1.reset_index()
grouped_prediction_final2 = (grouped_prediction.groupby(['DATETIME_PREDICTION', pd.Grouper(key='ALGORITHMS')]).agg({'PRICE_PREDICTION':'mean'}))
...
grouped_prediction_final = grouped_prediction_final[grouped_prediction_final["ALGORITHMS"].str.contains("DAILY") == False]
...
token_backend = FileSystemTokenBackend(token_path='O365_Token', token_filename='o365_token')
