...
eth_df = yf.download('ZEC-USD',start_date, today)
...
model = AutoTS(forecast_length=7, frequency='infer', ensemble='simple', drop_data_older_than_periods=200 )
...
for index, row in df.iterrows():
    cursor.execute("INSERT INTO ZEC_v2_YAHOOFINANCE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'ZEC','AUTOTS_GLUONTS_7D','DAILY')", row.Date, row.Close)
    cnxn.commit()
cursor.close()
