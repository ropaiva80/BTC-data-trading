...
eth_df = yf.download('GALA-USD',start_date, today)
...
for index, row in df.iterrows():
    cursor.execute("INSERT INTO GALA_v2_YAHOOFINANCE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'ETHEREUM','AUTOTS_GLUONTS_ON','DAILY')", row.Date, row.Close)
    cnxn.commit()
cursor.close()
