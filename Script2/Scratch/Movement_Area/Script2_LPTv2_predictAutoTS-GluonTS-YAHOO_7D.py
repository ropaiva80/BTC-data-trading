...
eth_df = yf.download('LPT-USD',start_date, today)
...
for index, row in df.iterrows():
    cursor.execute("INSERT INTO LPT_v2_YAHOOFINANCE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'LPT','AUTOTS_GLUONTS_7D','DAILY')", row.Date, row.Close)
    cnxn.commit()
cursor.close()
