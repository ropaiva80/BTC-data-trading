...
new = HistoricalData('LPT-USD',3600,start_date,end_date).retrieve_data()
new = new.reset_index()
...
for index, row in df.iterrows():
	cursor.execute("INSERT INTO LPT_v1_COINBASE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'LPT','AUTOTS_GLUONTS_OFF','HOURLY')", row.time, row.close)
cnxn.commit()
cursor.close()
