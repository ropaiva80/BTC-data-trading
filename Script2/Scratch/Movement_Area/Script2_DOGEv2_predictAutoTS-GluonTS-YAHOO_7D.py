...
# Turn on script execution (PS): Set-ExecutionPolicy -Scope CurrentUser 0
...
eth_df = yf.download('DOGE-USD',start_date, today)
...
print("Etherium Price Prediction")
print(forecast)
...
for index, row in df.iterrows():
    cursor.execute("INSERT INTO DOGECOIN_v2_YAHOOFINANCE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'DOGECOIN','AUTOTS_GLUONTS_ON','DAILY')", row.Date, row.Close)
    cnxn.commit()
cursor.close()
