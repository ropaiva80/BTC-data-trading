...
end_date = (today)
new = HistoricalData('ZEC-USD',3600,start_date,end_date).retrieve_data()
new = new.reset_index()
...
#model = AutoTS(forecast_length=12, frequency='infer', ensemble='simple', model_list=modelX, transformer_list="fast", validation_method="backwards")
...
for index, row in df.iterrows():
	cursor.execute("INSERT INTO ZEC_v1_COINBASE (DATETIME_PREDICTION,PRICE_PREDICTION,ASSET_NAME,ALGORITHMS,CATEGORY) values(?,?,'ZEC','AUTOTS_GLUONTS_OFF','HOURLY')", row.time, row.close)
cnxn.commit()
cursor.close()